...
export default function ActivityTimeline({ leadId, onActivityLogged }: ActivityTimelineProps) {
  ...
  return (
    <div>
      ...
      {/* Log Activity Form */}
      {showForm && (
        <div className="mb-3 p-3.5 bg-gray-50 rounded-xl border border-gray-200 space-y-2.5">
            ...
            {ACTIVITY_TYPES.map(t => (
              <button
                ...
                className={cn(
                  ...
                  formType === t.value
                    ? cn(t.color, 'ring-1 ring-current shadow-sm')
          ...
          <textarea
            ...
            rows={2}
            className="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/40 focus:border-blue-300 resize-none bg-white transition-all"
            onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSubmit(); } }}
          />
          <div className="flex justify-end gap-2">
            <button onClick={() => setShowForm(false)} className="px-3 py-1.5 text-xs text-gray-500 hover:text-gray-700 rounded-lg hover:bg-gray-100 transition-colors">Cancel</button>
            <button
              onClick={handleSubmit}
              disabled={!formDescription.trim() || submitting}
              className="flex items-center gap-1 px-3 py-1.5 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-lg text-xs font-medium hover:shadow-md disabled:opacity-50 transition-all shadow-sm"
            >
      ...
      {loading ? (
        ...
        <div className="space-y-0 relative">
          <div className="absolute left-[15px] top-2 bottom-2 w-px bg-gradient-to-b from-gray-200 to-transparent" />
          {activities.slice(0, 20).map(activity => {
            const style = getActivityStyle(activity.type);
            const Icon = style.icon;
            return (
              <div key={activity.id} className="flex items-start gap-2.5 py-1.5 relative group">
                <div className={cn(
                  'w-[30px] h-[30px] rounded-xl flex items-center justify-center flex-shrink-0 border z-10 transition-all group-hover:shadow-sm',
                  style.color
                )}>
